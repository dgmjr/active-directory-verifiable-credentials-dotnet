            ...
            try
            {
                ...
                if (!System.IO.File.Exists(payloadpath))
                {
                    ...
                    return BadRequest(
                        ...
                        {
                            error = "400",
                            error_description = PRESENTATIONPAYLOAD + " not found"
                        }
                    );
                }
                ...
                if (string.IsNullOrEmpty(jsonString))
                {
                    ...
                    return BadRequest(
                        ...
                        {
                            error = "400",
                            error_description = PRESENTATIONPAYLOAD + " error reading file"
                        }
                    );
                }
                ...
                {
                    payload["requestedCredentials"][0]["acceptedIssuers"][0] =
            ...
            try
            {
                ...
                if (presentationResponse["requestStatus"].ToString() == "presentation_verified")
                {
                    var cacheData = new
                    {
                        ...
                        firstName = presentationResponse["verifiedCredentialsData"][0]["claims"][
                            "firstName"
                        ].ToString(),
                        lastName = presentationResponse["verifiedCredentialsData"][0]["claims"][
                            "lastName"
                        ].ToString()
                    };
                    _cache.Set(state, JsonConvert.SerializeObject(cacheData));
