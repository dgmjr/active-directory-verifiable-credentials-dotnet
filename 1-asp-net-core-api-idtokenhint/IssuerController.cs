    ...
    public class IssuerController : ControllerBase
    {
        ...
        public async Task<ActionResult> IssuanceRequest()
        {
            try
            {
                ...
                {
                    string errmsg =
                        $"TenantId in ManifestURL {manifestTenantId}. does not match tenantId in config file {AppSettings.TenantId}";
                ...
                try
                {
                    ...
                    if (accessToken.Item1 == String.Empty)
                    {
                        _log.LogError(
                            String.Format(
                                "failed to acquire accesstoken: {0} : {1}",
                                accessToken.error,
            ...
            try
            {
                if (!_cache.TryGetValue("manifest", out string manifest))
                {
                    ...
                    HttpResponseMessage res = client
                        .GetAsync(AppSettings.CredentialManifest)
                        .Result;
                    ...
                    manifest = System.Text.Encoding.UTF8.GetString(
                        Convert.FromBase64String(jwtToken)
                    );
