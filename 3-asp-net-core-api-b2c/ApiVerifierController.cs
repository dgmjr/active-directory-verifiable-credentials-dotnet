...
namespace AspNetCoreVerifiableCredentialsB2C
{
    ...
    public class ApiVerifierController : ApiBaseVCController
    {
        ...
        protected JObject GetPresentationManifest()
        {
            ...
            {
                _log.LogError(
                    "HttpStatus {0} fetching manifest {1}",
                    statusCode,
                    this.AppSettings.DidManifest
                );
                return null;
            }
        ...
        protected VCPresentationRequest CreatePresentationRequest(string correlationId)
        {
            ...
            request.requestedCredentials.Add(
                new RequestedCredential()
                {
                    ...
                    acceptedIssuers = new List<string>(
                        new string[] { this.AppSettings.IssuerAuthority }
                    )
                }
            );
            return request;
        ...
        public ActionResult PresentationReference()
        {
            ...
            try
            {
                ...
                if (
                    !HttpPost(
                        this.AppSettings.ApiEndpoint + "createPresentationRequest",
                        jsonString,
                        out statusCode,
                        out contents
                    )
        ...
        public ActionResult StaticPresentationReferenceGet()
        {
            ...
            try
            {
                ...
                if (
                    ...
                    && !string.IsNullOrWhiteSpace(stateProp)
                )
                {
                    stateProp = stateProp.PadRight(stateProp.Length + (stateProp.Length % 4), '=');
                    JObject spJson = JObject.Parse(
                        Encoding.UTF8.GetString(Convert.FromBase64String(stateProp))
                    );
                ...
                var resp = new
                {
                    ...
                    url = string.Format(
                        "openid://vc/?request_uri={0}/presentation-request-proxy?id={1}",
                        GetApiPath(),
                        correlationId
                    ),
                    expiry = (int)
                        (DateTime.UtcNow.AddDays(1) - new DateTime(1970, 1, 1)).TotalSeconds,
                    id = correlationId
                };
            ...
            try
            {
                ...
                if (GetCachedObject<VCCallbackEvent>(correlationId, out VCCallbackEvent callback))
                {
                    ...
                    if (callback.requestStatus == "presentation_verified")
                    {
                        ...
                        JObject resp = JObject.Parse(
                            JsonConvert.SerializeObject(new { status = 2, message = displayName })
                        );
                        foreach (
                            KeyValuePair<string, string> kvp in callback.verifiedCredentialsData[
                                0
                            ].claims
                        )
                        {
                ...
                {
                    return ReturnJson(
                        JsonConvert.SerializeObject(new { status = 0, message = "No data" })
                    );
                }
        ...
        public ActionResult PresentationResponseB2C()
        {
            ...
            try
            {
                ...
                var obj = new
                {
                    vcType = callback.verifiedCredentialsData[0].type[
                        callback.verifiedCredentialsData[0].type.Length - 1
                    ], // last
                ...
                foreach (
                    KeyValuePair<string, string> kvp in callback.verifiedCredentialsData[0].claims
                )
                {
