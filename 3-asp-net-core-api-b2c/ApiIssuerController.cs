...
namespace AspNetCoreVerifiableCredentialsB2C
{
    ...
    public class ApiIssuerController : ApiBaseVCController
    {
        ...
        protected Dictionary<string, string> GetSelfAssertedClaims(JObject manifest)
        {
            ...
            if (
                manifest["input"]["attestations"]["idTokens"][0]["id"].ToString()
        ...
        public ActionResult IssuanceReference()
        {
            ...
            try
            {
                ...
                VCIssuanceRequest request = new VCIssuanceRequest()
                {
                    ...
                    registration = new Registration() { clientName = this.AppSettings.client_name },
                ...
                {
                    int pinCode = RandomNumberGenerator.GetInt32(
                        1,
                        int.Parse("".PadRight(this.AppSettings.IssuancePinCodeLength, '9'))
                    );
                    ...
                    request.pin = new Pin()
                    {
                        ...
                        value = string.Format(
                            "{0:D" + this.AppSettings.IssuancePinCodeLength.ToString() + "}",
                            pinCode
                        )
                    };
                }
                ...
                if (
                    !HttpPost(
                        this.AppSettings.ApiEndpoint + "createIssuanceRequest",
                        jsonString,
                        out statusCode,
                        out contents
                    )
            ...
            try
            {
                ...
                if (GetCachedObject<VCCallbackEvent>(correlationId, out VCCallbackEvent callback))
                {
                    ...
                    if (callback.requestStatus == "issuance_successful")
                    {
                        ...
                        return ReturnJson(
                            JsonConvert.SerializeObject(
                                new { status = 2, message = "Issuance process is completed" }
                            )
                        );
                    }
                    if (callback.requestStatus == "issuance_error")
                    {
                        ...
                        return ReturnJson(
                            JsonConvert.SerializeObject(
                                ...
                                {
                                    status = 99,
                                    message = "Issuance process failed with reason: "
